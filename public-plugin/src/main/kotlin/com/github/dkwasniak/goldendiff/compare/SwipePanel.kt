...
class SwipePanel : JPanel(BorderLayout()) {
    ...
    private class Canvas : ZoomablePanel() {
        ...
        private fun imageArea(): Rectangle = Rectangle(0, titleHeight, width, height - titleHeight)

        private fun scale(): Double {
            val (w, h) = ImagePainting.boundingSize(oldImage, newImage)
            val area = imageArea()
            return ImagePainting.scaleForBounding(zoom, w, h, area.width, area.height)
        }

        /** Bounding reference rect (bottom-anchored) used for the divider, checkerboard and labels. */
        private fun currentRect(): Rectangle {
            val (w, h) = ImagePainting.boundingSize(oldImage, newImage)
            return ImagePainting.bottomRect(w, h, scale(), imageArea())
        }
        ...
        override fun paintComponent(g: Graphics) {
            ...
            try {
                ...
                newImage?.let { ImagePainting.drawImage(g2, it, ImagePainting.bottomRect(it.width, it.height, scale, area)) }
                ...
                oldImage?.let {
                    ...
                    try {
                        ...
                        ImagePainting.drawImage(clip, it, ImagePainting.bottomRect(it.width, it.height, scale, area))
                    } finally {
                        clip.dispose()
                    }
                }
