    ...
    private class Canvas : ZoomablePanel() {
        ...
        override fun paintComponent(g: Graphics) {
            ...
            try {
                ...
                val rect = ImagePainting.bottomRect(bw, bh, scale, area)
                if (rect.width <= 0) return
                drawLegend(g2, rect)
                ImagePainting.paintCheckerboard(g2, rect)
                oldImage?.let { ImagePainting.drawImage(g2, it, ImagePainting.bottomRect(it.width, it.height, scale, area)) }
                newImage?.let {
                    g2.composite = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha)
                    ImagePainting.drawImage(g2, it, ImagePainting.bottomRect(it.width, it.height, scale, area))
                }
            } finally {
                g2.dispose()
            }
