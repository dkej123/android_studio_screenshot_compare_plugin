...
class FigmaReferenceDownloader(
    ...
    fun download(preview: FigmaPreview, target: FigmaTarget, output: File): FigmaDownloadResult {
        ...
        return try {
            // The node tree (/nodes) and the image render (/images) are independent — both only
            // need the node id — and each is several seconds on a cold Figma cache. Fetch them
            // concurrently so the total is max(nodes, render) instead of their sum.
            val (frame, imageUrl) = fetchInParallel(
                { fetchFrame(target, token) },
                { fetchImageUrl(target, token) },
            )
            frame ?: return FigmaDownloadResult.Failure(
            ...
            imageUrl ?: return FigmaDownloadResult.Failure(
...
/**
 * Run two independent blocking calls concurrently and return their results as a pair, preserving
 * argument order. [first] runs on the calling thread while [second] runs on a worker, so the total
 * wait is roughly the slower of the two rather than their sum. An exception from either call is
 * rethrown unwrapped (the [ExecutionException] wrapper is peeled off).
 */
internal fun <A, B> fetchInParallel(first: () -> A, second: () -> B): Pair<A, B> {
    val pool = Executors.newSingleThreadExecutor()
    return try {
        val secondFuture = pool.submit(Callable { second() })
        val firstResult = first()
        try {
            firstResult to secondFuture.get()
